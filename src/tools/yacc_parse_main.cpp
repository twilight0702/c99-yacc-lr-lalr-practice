...
std::vector<std::string> collect_declared_token_names(const seu::yacc::Grammar& grammar) {
    std::vector<std::string> names;
    for (int symbol_id : grammar.terminal_ids) {
        if (symbol_id < 0 || symbol_id >= static_cast<int>(grammar.symbols.size())) {
            continue;
        }
        const auto& symbol = grammar.symbols[symbol_id];
        if (symbol.is_literal_char) {
            continue;
        }
...
void emit_y_tab_header(const seu::yacc::Grammar& grammar, const std::string& output_path) {
    const std::vector<std::string> token_names = collect_declared_token_names(grammar);
    ...
    out << "\n";
    out << "#define YYEMPTY (-2)\n";
    out << "#define YYEOF 0\n";
    out << "#define YYerror 256\n";
    out << "#define YYUNDEF 257\n";
    for (const auto& name : token_names) {
        out << "#define " << name << " " << name << "\n";
    }
    out << "\n";

...
- `run` 且提供 `--parse-tokens` 时，固定按 bison 风格执行 `.y` 归约动作（编译后执行）。
- 不提供动作执行开关参数（无轻量/全量模式分支）。
...
- 动作执行只在 LALR 第9步发生（LR1 路径不执行动作），避免双跑造成重复副作用。
- 该能力会执行真实代码，请仅在可信文法下使用。
- 若动作代码不可编译，工具会报错并提示查看 `/tmp/yacc_action_build_stderr.log`。
- 已接入 bison 风格占位符绑定：`$1`/`$2`/`$$` 与 `@1`/`@$`（会在动作代码中展开为语义值与位置信息访问）。
- 当前尚未完整覆盖 bison 的高级占位符语法（例如复杂的 `$<tag>n` 组合与所有默认转换语义），建议优先使用 `%token/%type` 的类型标签和基础占位符形式。

`simple_test/cal.y` 这类未声明 `%union` 的文法按 bison 默认 `YYSTYPE=int` 语义处理；
例如输入 `1+2*3/2` 与 `2*7-6+5`，动作输出期望为 `= 4`、`= 13`。
...
- 扩展语义动作执行能力（当前仅支持输出类动作），并与 AST 构建器（Step11）绑定
